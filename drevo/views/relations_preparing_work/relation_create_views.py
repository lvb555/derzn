...
@login_required
def insert_knowledge_view(request, rz_id):
    """
    Вставка нового знания между базовым и связанным знанием
    """
    ...
    if request.method == 'POST':
        form = AdditionalKnowledgeForm(request.POST)
        if form.is_valid():
            with transaction.atomic():
                # Создаем новое знание
                new_knowledge = form.save(commit=False)
                new_knowledge.author = request.user.author
                new_knowledge.save()

                # Связь между базовым и новым знанием
                ...
                # Связь между новым и выбранным знанием (вставка)
                Relation.objects.create(
                    bz=new_knowledge,
                    rz=rz,
                    tr=get_object_or_404(Tr, name='Вставка'),
                    author=request.user.author,
                    user=request.user
                )

                return redirect('relation_create_page', base_kn=bz_id)

    else:
        form = AdditionalKnowledgeForm(initial={'name': f'Вставка для {rz.name}'})

    return render(request, 'drevo/insert_knowledge_page.html', {
        'form': form,
        'rz': rz,
        'bz_id': bz_id,
        'tr_id': tr_id,
        'back_url': reverse('relation_create_page') + f'?base_kn={bz_id}'
    })