...
def dimensional_distributions_2(request):
    ...
    request.session['selected_interview'] = selected_interview
    question1 = request.GET.get('selected_question_1')
    request.session['selected_question_1'] = question1
    request.session.save()
    question2 = request.GET.get('selected_question_2')

    if selected_interview is None:
        interview_object = InterviewAnswerExpertProposal.objects.filter(question__name=question1).first()
        if interview_object:
    ...
    ).values_list('question__name', flat=True)))
    question1 = request.session.get('selected_question_1')
    ...
    for answer2 in answers_2:
        total_counts[answer2] = InterviewAnswerExpertProposal.objects.filter(
            ...
            is_agreed=True
        ).values('expert__id').distinct().count()  # подсчет уникальных экспертов
    ...
    for expert_1 in experts_agreed_1:
        ...
        for expert_2 in experts_agreed_2:
            ...
            if expert_1['expert__id'] == expert_2['expert__id']:
                ...
                common_experts[answer_1][answer_2]['percentage'] = (common_experts[answer_1][answer_2]['count'] / common_experts[answer_1][answer_2]['total']) * 100 if common_experts[answer_1][answer_2]['total'] > 0 else 0
    ...
    for answer1 in answers_1:
        ...
        for answer2 in answers_2:
            ...
            row['totals'].append(total_counts[answer2])  # добавьте общее количество голосов для этого ответа
        table.append(row)
