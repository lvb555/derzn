...
class KnowledgeTreeBuilder:
    ...
    def _build_tree_data(self, knowledge_list: list) -> None:
        ...
        Для каждого узла из knowledge_list:
          - Если узел является «сложным» и self.show_complex=False — прерывается обработка цепочки.
          - Проверяем глобальную карту: если узел уже встречался, используем его поддерево,
            иначе создаём новый пустой словарь для поддерева и сохраняем в карте.
          - Затем прикрепляем поддерево к текущему локальному уровню.

        Это обеспечивает, что если узел (например, 'В') уже построен с поддеревом (например, {'Г': {}}),
        то при повторном появлении этот же словарь подключается и для другой ветви.
        """
        ...
        for node in knowledge_list:
            ...
            # Присоединяем существующий subtree к текущему локальному уровню.
            found = False
            for existing_node in current_level:
                if existing_node == node:
                    # Узел уже присутствует в локальном уровне, используем его поддерево.
                    subtree = current_level[existing_node]
                    found = True
                    break
            if not found:
