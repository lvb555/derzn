...
let delete_element_modal = $("#delete_element")
...
edit_main_zn_form.on('submit', (event) => {
    ...
    .then(response => response.json())
    .then(data => {
        $(`#table option[id="${data.zn_id}"]`).text(data.zn_name);
    ...
    if (table_id) {
        ...
        $('.js-okay-successful').click(function () {
            ...
            .then(response => {
                ...
                $('#open_table').hide();
                let successful_table_delete = `Таблица «${$('#table option:selected').text().trim()}» успешно удалена!`;
                    $('.constructor-block').empty().append(
                        `<span>${successful_table_delete}</span>`
                    )
...
delete_row.on('click', function(){
    let row_id = row.val()
    if (row_id) {
        $('.delete-confirmation').text(`Вы действительно хотите удалить данную строку? Все связанные знания также удалятся!`);
        delete_element_modal.modal("show");
        $('.js-okay-successful').click(function () {
...
delete_column.on('click', function(){
    ...
    if (column_id) {
        $('.delete-confirmation').text(`Вы действительно хотите удалить данный столбец? Все связанные знания также удалятся!`);
        delete_element_modal.modal("show");
        $('.js-okay-successful').click(function () {
...
function deleteRelation(relation, is_group) {
    ...
    fetch(`/drevo/delete_row_or_column/?id=${relation_id}&is_group=${is_group.val()}`)
    .then(response => {
        if (relation === 'row') {
