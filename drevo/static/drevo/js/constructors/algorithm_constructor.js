...
function add_relation(parent_zn_id) {
    fetch(`/drevo/rel_for_algorithm/create/?parent_id=${parent_zn_id}`)
        .then(response => {
        ...
        .then(data => {
            if (data['relations'].length === 0) {
                $('#error_create_relation_modal').modal('show');
                $('#error_create_relation_message').text(`Для базового знания ${data['bz']['name']} уже создано максимальное количество связей! Вы можете удалить существующие свзязи для добавления новой.`)
            }
...
function edit_relation(rel_id) {
    fetch(`/drevo/rel_for_algorithm/edit/?rel_id=${rel_id}`)
...
tr.change(function() {
    let bz_id = $('#bz').val();
    let tr_id = $('#tr').val();
    fetch(`/drevo/get_rel_zn_in_algorithm/?bz_id=${bz_id}&tr_id=${tr_id}`)
        .then(response => response.json())
        .then(data => {
...
function delete_relation(bz_id, rz_id) {
    ...
    $('.js-okay-successful').click(function () {
        fetch(`/drevo/delete_relation_in_algorithm/?bz_id=${bz_id}&rz_id=${rz_id}`, {
         method: 'DELETE',
         headers: {
...
create_zn.addEventListener('submit', (event) => {
    ...
    fetch('/drevo/create_zn_in_algorithm/', {
     method: 'POST',
     body: formData,
    })
    .then(response => response.json())
...
function edit_znanie(for_type) {
    ...
    fetch(`/drevo/edit_znanie_in_algorithm/?zn_id=${zn_id}`)
    .then(response => response.json())
    .then(data => {
...
edit_zn.addEventListener('submit', (event) => {
    ...
    fetch(`/drevo/edit_znanie_in_algorithm/`, {
     method: 'POST',
     body: formData,
    })
    .then(response => response.json())
...
$('#save_new_relation').click(function () {
    if (bz.val() && tr.val() && rz.val()) {
        ...
        if (current_action.val() === 'create') data = { bz_id: $('#bz').val(), tr_id: $('#tr').val(),
            rz_id: $('#rz').val(), action: 'create'};
        else data = { bz_id: $('#bz').val(), tr_id: $('#tr').val(),  rz_id: $('#rz').val(),
            last_tr_id: last_tr_id.val(), last_rz_id: last_rz_id.val(), action: 'edit'};

         fetch('/drevo/save_rel_in_algorithm/', {
             method: 'POST',
...
function copy(zn_id) {
    ...
    .then(data => {
        ...
        if (data.id)
            window.open(`/drevo/algorithm_constructor/${data.id}`)
    })
}