...
class User(AbstractUser):
    ...
    def send_verify_mail(self):
        verify_link = reverse('profiles:verify', args=[self.username, self.activation_key])
        subject = 'Активация аккаунта'
        message = f'Чтобы активировать аккаунт, перейдите по ссылке: ' \
                  f'{settings.BASE_URL}{verify_link}'
        return send_mail(subject, message, f'Дерево знаний <{settings.EMAIL_HOST_USER}>', [self.email])
    ...
    def send_password_recovery_mail(self):
        recovery_link = reverse('profiles:password-recovery-link', args=[self.email, self.password_recovery_key])
